...
import { getFirstFile } from './models/util';
...
const SessionSelect: React.FC<Props> = ({
  setCurSession,
  setSelectedPreset,
}) => {
  ...
  const selectSession = (opt: Option<string>) => {
    (async () => {
      ...
      if (session) {
        ...
        setSelectedPreset(
          session.presets.filter((x) => x.type === session.presetMode)[0],
        );
      }
    })();
  };
                  ...
                  callback: async (inputValue) => {
                    if (inputValue) {
                      ...
                      if (tarPath) {
                        ...
                        setCurSession(sess);
                        ctx.setSelectedPreset(
                          sess!.presets.filter(
                            (x) => x.type === sess!.presetMode,
                          )[0],
                        );
                      }
                    }
