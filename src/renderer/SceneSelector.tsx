...
interface SceneSelectorProps {
  text: string;
  scenes: Scene[];
  getImage: (scene: GenericScene) => Promise<string>;
  onConfirm: (selectedScenes: Scene[]) => void;
}

const SceneImage: React.FC<{ scene: GenericScene, getImage: (scene: GenericScene) => Promise<string> }> = ({ scene, getImage }) => {
      ...
      <div className="px-1 pt-3 flex flex-col flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden pt-4 pb-2">
          <div className="flex flex-wrap h-full overflow-auto gap-2">
            {scenes.map(scene => (
              <div className={"p-2 border border-gray-400 flex flex-col items-center " + (selectedScenes.some(selected => selected.name === scene.name) ? "border-sky-500 bg-sky-200":"")}
