  ...
  return (
    <div className="p-2 md:p-4 flex flex-col h-full">
      <div className="flex items-center gap-2 text-gray-700 dark:text-gray-200 flex-none">
        ...
        <div className="text-lg md:text-xl flex flex-col md:flex-row md:gap-2"> <span>씬을 선택하고 해당 작업을 적용합니다:</span> <span className="font-bold text-default">{text}</span></div>
      </div>
      <div className="px-1 pt-2 md:pt-3 flex flex-col flex-1 overflow-hidden">
        <div className="gap-2 flex flex-none overflow-hidden">
          <button className={`round-button back-sky`} onClick={selectAllScenes}>모두 선택</button>
          <button className={`round-button back-gray`} onClick={clearAllSelections}>모두 선택 해제</button>
        </div>
        ...
        <div className="flex-none flex">
          <button className={`round-button back-green ml-auto`} onClick={() => onConfirm(selectedScenes)}>작업 적용</button>
        </div>
      </div>
    </div>
  );
