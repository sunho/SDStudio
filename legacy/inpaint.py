...
async def inpaint(prompt, uc, output_file_path, landscape, seed, vibe_image, image, mask):
    ...
    model = ImageModel.Inpainting_Anime_v3
    ...
    async for _, img in api.high_level.generate_image(prompt, model, preset, ImageGenerationType.INPAINTING):
...
def main():
    ...
    print(prompt)
    asyncio.run(inpaint(prompt, uc, output_file_path, landscape, seed, vibe_image, image, mask))
